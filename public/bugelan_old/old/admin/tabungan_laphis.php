...
$pdf->Cell(0,0.7,'REKAP TABUNGAN SISWA',0,1,'C');
...
$pdf->Cell(9, 0.7, 'Nama Siswa', 1, 0, 'C');
$pdf->Cell(2, 0.7, 'Debit', 1, 0, 'C');
$pdf->Cell(2, 0.7, 'Kredit', 1, 0, 'C');
$pdf->Cell(3, 0.7, 'Admin', 1, 1, 'C');

$no=1;
$query=mysqli_query($conn, "SELECT * from tabungan where tgl between '$awl' and '$ahr' order by tgl");
while($lihat=mysqli_fetch_assoc($query)){
$pdf->SetFont('Arial','',8);
	$pdf->Cell(1, 0.7, $no,1, 0, 'C');
	$pdf->Cell(2, 0.7, $lihat['tgl'],1, 0, 'C');
	$pdf->Cell(9, 0.7, substr($lihat['nama'], 0, 20), 1, 0,'L');
...
$pdf->ln(0.25);
$pdf->SetFont('Arial','B',8);
$tot=mysqli_query($conn, "SELECT sum(debit) as dbt, sum(kredit) as kdt from tabungan where tgl between '$awl' and '$ahr' ");
