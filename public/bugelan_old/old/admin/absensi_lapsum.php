...
$pdf->Cell(2, 1.4, 'NISN', 1, 0, 'C');
$pdf->Cell(1, 1.4, 'Kls', 1, 0, 'C');
$pdf->Cell(6.5, 1.4, 'Nama Siswa', 1, 0, 'C');
...
while($lihat=mysqli_fetch_assoc($query)){
	...
	$pdf->Cell(6.5, 0.7, substr($lihat['nama'], 0, 30), 1, 0,'L');
	
$ids=$lihat['id'];
$jml=mysqli_query($conn, "SELECT COUNT(IF(ket='', time, NULL)) AS h, COUNT(DISTINCT IF(ket='i', ket, NULL)) AS i, COUNT(DISTINCT IF(ket='s', ket, NULL)) AS s, COUNT(DISTINCT IF(ket='a', ket, NULL)) AS a, SUM(LEFT(late,2)) AS hor, SUM(MID(late,4,2)) AS min, SUM(RIGHT(late,2)) AS sec FROM absen WHERE id_siswa='$ids'");
