...
class Adm extends CI_Controller
{
	...
	public function m_soal()
	{
		...
		if ($uri3 == "det") {
			...
			$q_datanya = $this->db->query("SELECT a.*, b.nama nmguru, c.id idmapel, c.nama nmmapel, d.id idkelas, d.kelas nmkelas
												FROM m_soal a
												INNER JOIN m_guru b ON a.id_guru = b.id
												INNER JOIN m_mapel c ON a.id_mapel = c.id
												INNER JOIN m_kelas d ON a.id_kelas = d.id
		                                        ...
		                                        ORDER BY idmapel, idkelas, a.id LIMIT " . $start . ", " . $length . "")->result_array();
			//echo $this->db->last_query();
			$data = array();
			$no = ($start + 1);
			foreach ($q_datanya as $d) {
